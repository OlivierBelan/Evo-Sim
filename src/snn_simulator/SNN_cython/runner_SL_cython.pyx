...
cdef class Runner_SL_cython:
    ...
    @cython.boundscheck(False)
    @cython.wraparound(False)
    cpdef void init_augmented_spikes_decoder(self, size_t spike_max = 10, size_t spike_distribution_run = 10, size_t spike_distribution_importance = 10, str importance_type = "nothing", str linear_spike_importance_type = "descending", str spike_type = "positive", bint is_normalize = False, bint is_interpolate = False, float interpolate_max = 1.0, float interpolate_min = 0.0, bint is_voltage_reset=True):
    ...
    cdef void record(self, list snn):
        ...
        if self.is_record_augmented: 
            ...
            # print("spike_sumed_decoder\n", np.array(spike_sumed_decoder), "shape", np.shape(spike_sumed_decoder), "max", np.max(spike_sumed_decoder), "min", np.min(spike_sumed_decoder))
