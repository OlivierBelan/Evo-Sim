...
class HyperNEAT(Algorithm):
    def __init__(self, config_path_file:str, cppn_synpases_config_path:str, cppn_neurons_config_path:str, substrats:List[np.ndarray], substrats_connection:List[Tuple[int, int]], name:str = "HyperNEAT") -> None:
        ...
        self.population_decoder:Population = Population(get_new_population_id(), config_path_file)
        ...
        # Initialize Decoder algorithm
        self.decoder_algorithm:Decoder_GA = Decoder_GA(config_path_file, name="Decoder_GA")
        # self.decoder_algorithm:Decoder_CMA = Decoder_CMA(config_path_file, name="Decoder_CMA")
        ...
        if self.cppn_neurons_algorithm.pop_size != self.decoder_algorithm.pop_size: raise Exception("HyperNEAT: CPPN_neurons pop_size (" + str(self.cppn_neurons_algorithm.pop_size) + ") must be equal to Decoder pop_size (" + str(self.decoder_algorithm.pop_size) +")")
    ...
    def run_neurons_cppn(self, genomes_from_cppn:Dict[int, Genome]) -> None:
        # 1 - Run CPPN
        self.cppn_neurons_record:Dict[int, np.ndarray] = self.cppn_neurons_runner.run_snns(genomes_from_cppn, self.substrat_neurons_configuration)
        ...
        # print("self.cppn_neurons_record", self.cppn_neurons_record)
        # print("self.cppn_neurons_record[self.cppn_neurons_output_indexes]", self.cppn_neurons_record[0][:, self.cppn_neurons_output_indexes])
        # exit()

    def run_synpases_cppn(self, genomes_from_cppn:Dict[int, Genome]) -> None:
        
        # 1 - Run CPPN
        self.cppn_synapses_record:Dict[int, np.ndarray] = self.cppn_synpases_runner.run_snns(genomes_from_cppn, self.substrat_synpases_configuration)
        ...
        # print("self.cppn_synapses_output_indexes", self.cppn_synapses_record)
        # print("self.cppn_synapses_record[self.cppn_neurons_output_indexes]", self.cppn_synapses_record[first_key][:, self.cppn_synapses_output_indexes])
        # exit()
    ...
    def run(self, global_population:Population, evaluation_function) -> Population:
        ...
        self.decoder_algorithm.run(self.population_decoder)
    ...
    def __build_HyperNEAT_population(self):
        ...
        decoder_population:List[Genome_Decoder] = list(self.population_decoder.population.values())
        ...
        for index in range(self.pop_size):
            ...
            if index == 0:
                print("decoder_neuron", decoder_population[index].decoder_neuron)
                print("decoder_synapse", decoder_population[index].decoder_synapse)
            if hyperneat_population[index].nn.network_type != "SNN": raise Exception("HyperNEAT: NN network_type (" + str(hyperneat_population[index].nn.network_type) + ") must be equal to SNN")
            self.__build_snn_from_cppn(
                ...
                decoder_population[index].decoder_neuron,
                decoder_population[index].decoder_synapse,
    ...
    def __syncronize_fitness(self, population_manager:Population) -> None:
        ...
        decoder_pop:Dict[int, Genome] = self.population_decoder.population
        for hyperneat, neuron, synapse, decoder in zip(hyperneat_pop.values(), cppn_neurons_pop.values(), cppn_synapses_pop.values(), decoder_pop.values()):
            neuron.fitness.score = hyperneat.fitness.score
            synapse.fitness.score = hyperneat.fitness.score
            decoder.fitness.score = hyperneat.fitness.score
